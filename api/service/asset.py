...
def get_untreated_servers():
    ...
    try:
        current_date = datetime.date.today()  # 获取当前的日期
        ...
        con_date.children.append(("asset__latest_date__lt", current_date))  # 资产记录最新时间小于当前时间
        con_date.children.append(("asset__latest_date", None))  # 资产记录最新时间为空；

        # 在线状态的服务器
        con_status = Q()
        con_status.children.append(('asset__device_status_id', '2'))  # 资产设备当前在线；

        condition.add(con_date, 'AND')
        condition.add(con_status, 'AND')

        result = models.Server.objects.filter(condition).values('hostname')
        response.data = list(result)  # result是queryset类型，里面是一个个字典[{'hostname':'xx','hostname':'xx' }]，通过List转换成列表
