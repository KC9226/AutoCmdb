(function (jq) {
            ...
            $.each(tbConfig, function (k2, config) {
                ...
                if (config.display) {
                    ...
                    $.each(config.text.kwargs, function (k, v) {
                        if (v.startsWith('@@@')) {
                            var global_name = v.substring(3, v.length);
                            kwargs[k] = getNameByGlobalList2(global_name, row[config.q]);
                    ...
                    $.each(kwargs, function (k, v) {
                        if (v instanceof Array){
                                //var span_str = '';
                                var span_list = new Array();
                                $.each(v, function (k2, r) {
                                    var span = document.createElement('span');
                                    kwargs[k] = r;
                                    span.innerHTML = config.text.content.format(kwargs) + '|';
                                    span_list.push(span)
                                    //span_str += span
                                });
                                console.log('span_list', span_list);
                                for(var i =0; i<span_list.length;i++){
                                    td.append(span_list[i])
                                }
                        } else {
                            td.innerHTML = config.text.content.format(kwargs);
                        }
    ...
    function getNameByGlobalList(globalName, itemId) {
        var result = '';
        console.log('dict:', NB_GLOBAL_DICT, itemId);
    ...
    function getNameByGlobalList2(globalName, itemId) {
        var result = new Array();
        console.log('dict:', NB_GLOBAL_DICT, NB_GLOBAL_DICT[globalName], itemId);
        $.each(NB_GLOBAL_DICT[globalName], function (k, v) {
            if (v.id == itemId) {
                //result = v.usergroupname;
                result.push(v.usergroupname);
                //return result;
                //return false;
            }
        });
